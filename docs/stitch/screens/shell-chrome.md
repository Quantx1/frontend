...
## `components/shell/ContextPanel.tsx` — REMOVED (no longer in the codebase)

**This component no longer exists.** It was added at 3e437f2 to serve a rewritten `/markets`
that rendered a market brief plus a right-hand slide-over. That rewrite was reverted — `/markets`
is back to its full-width bento of cards (see [`markets-stocks.md`](markets-stocks.md)) — and
`ContextPanel.tsx`, `MarketPanel.tsx` (`useMarketPanelFaces`) and `RenderedThread.tsx` were deleted
with it, since nothing imported them any more.

There is now **no slide-over panel anywhere in the app**. Any redesign that wants a "Details"
overlay would be introducing a new pattern, not restoring an existing one. `RightRail.tsx` (the
72px icon rail) is unrelated and still very much present — see its own section above.
