...
## 6. `/markets` was reverted after these docs were generated

These documents were produced by reading the codebase at commit `3e437f2`, which had just rewritten
`/markets` into a deterministic copilot surface — a market brief, one EntityCard, four follow-up
chips, and a tabbed right slide-over holding the depth. **That rewrite was reverted.** `/markets` is
back to `c3dff00`: a full-width bento of twelve cards (AI Daily Briefing hero, index strip, Market
Pulse, AI Radar, Breadth, Regime gauge, Sector heatmap, Market Explainer, Movers, Sector Rotation,
Top headlines, Big deals, and gated Order-Flow).

Everything describing `/markets` has been rewritten to match — [`screens/markets-stocks.md`](screens/markets-stocks.md),
this file's siblings, and prompt §39 in [`04-STITCH-PROMPTS.md`](04-STITCH-PROMPTS.md), which is now
generated from the same text as the screen doc so the two cannot drift.

Three components were deleted with the revert and no longer exist anywhere:
`components/shell/ContextPanel.tsx`, `components/markets/MarketPanel.tsx` (`useMarketPanelFaces`) and
`components/copilot/RenderedThread.tsx`. **There is no slide-over panel anywhere in Quant X.** A
redesign that introduces one is adding a pattern, not restoring it.

### Live orphan: the `market_detail` template

`market_detail` is still registered in the backend (`TEMPLATES` holds five keys) and still present in
the frontend's `CopilotTemplate` union at `lib/copilot/transport.ts:104` — but **no UI requests it any
more.** The only callers were the four follow-up chips (Breadth / FII/DII flows / Sectors / What's
firing) inside the deleted `RenderedThread`. The server work behind it (`_detail_plan`, `_detail_prose`,
and the `_prose_breadth` / `_prose_flows` / `_prose_sectors` / `_prose_firing` writers) is intact and
tested, so this is reachable-but-unreferenced rather than broken. Either wire it to a new affordance
or retire it deliberately — it should not sit as a silent orphan.

`RenderedSurface` itself is NOT orphaned: it is live on `/portfolio` (`my_book`) and `/signals`
(`signals_today`).

### Inert prop: `RenderedSurface.onOpenDetails`

3e437f2 added an optional `onOpenDetails` to `RenderedSurface` so the deleted `RenderedThread` could
open the slide-over. Both remaining call sites — `app/portfolio/page.tsx:264` and
`components/signals/SignalsOverview.tsx:268` — render `<RenderedSurface template="…" />` with no
handler, so line 112's `onOpen={onOpenDetails ? … : undefined}` always resolves to `undefined` and
`TurnCard` renders no Details button. That is exactly the pre-3e437f2 behaviour, so this is three
lines of inert plumbing, not a regression. Left in place deliberately: it is type-safe, costs
nothing, and is the natural hook if a details affordance returns. Delete it if it is still unused
when the `market_detail` question above is settled.
