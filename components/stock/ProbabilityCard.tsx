...
function color(p: number | null) {
  ...
  if (p >= 60) return 'var(--color-up)'
  if (p >= 45) return 'var(--color-warning)'
          ...
          <div key={s.name} className="flex items-center justify-between px-4 py-2.5">
            <div className="min-w-0">
              <div className="flex items-center gap-2">
                ...
                {s.active_now && (
                  <span className="text-[8px] font-bold uppercase tracking-wider text-primary bg-primary/10 px-2 py-0.5 rounded-full">Active now</span>
                )}
              </div>
