...
import {
  Newspaper, ExternalLink, ArrowRight, Sparkles, Bot, Brain, ShieldCheck,
  ScanLine, Stethoscope, BarChart3, Layers,
} from '@/lib/icons'
...
function NewsCard({ n }: { n: NewsItem }) {
  ...
  return (
    <a
      ...
      className="group flex flex-col overflow-hidden rounded-xl border border-line bg-wrap transition-all duration-200 hover:-translate-y-0.5 hover:border-d-text-muted/30 hover:shadow-lg"
    >
      <div className="relative aspect-[16/9] w-full overflow-hidden">
        ...
        >
          ...
          <span className="text-[11px] font-semibold uppercase tracking-wider text-d-text-muted/70">{n.source}</span>
        </div>
        {showImg && (
...
export function MarketNewsFeed() {
  ...
  const regioned = filter === 'all' ? all : all.filter((n) => n.region === filter)
  // image-bearing items lead so the grid reads as a proper photo feed; keep the
  // rest (branded placeholder) so we still show a full, even set of rows.
  const withImg = regioned.filter((n) => n.image)
  const noImg = regioned.filter((n) => !n.image)
  const items = [...withImg, ...noImg].slice(0, 9)
  ...
  return (
    <section aria-label="Market news">
      <header className="mb-3 flex flex-wrap items-center justify-between gap-2">
        ...
        <div className="flex items-center gap-0.5 rounded-lg border border-line bg-wrap p-0.5">
          {(['all', 'India', 'Global'] as Filter[]).map((f) => (
            <button
              key={f}
              onClick={() => setFilter(f)}
              className={`rounded-md px-2.5 py-1 text-[11px] font-medium transition-colors ${
                filter === f ? 'bg-primary/15 text-primary' : 'text-d-text-muted hover:text-d-text-secondary'
              }`}
            >
      ...
      {isLoading && all.length === 0 ? (
        ...
        <div className="rounded-xl border border-line bg-wrap p-12 text-center text-[12px] text-d-text-muted">
...
const FEATURE_LINKS = [
  ...
  { icon: BarChart3, title: 'ML Signals', desc: 'Gated NSE calls across horizons', href: '/signals' },
...
const CTA_FEATURES = [
  ...
  { icon: Brain, text: '5 ML engines must agree before a signal fires' },
...
function FeatureCard({ icon: Icon, title, desc, href }: (typeof FEATURE_LINKS)[number]) {
  return (
    <Link
      href={href}
      className="group flex items-center gap-3 rounded-xl border border-line bg-wrap px-3.5 py-3 transition-all duration-200 hover:-translate-y-0.5 hover:border-d-text-muted/30 hover:shadow-lg"
    >
      <span className={`grid h-9 w-9 shrink-0 place-items-center rounded-lg bg-white/[0.05] ${featureAccent(href)}`}>
...
export function HomeCtaBand() {
  return (
    <div className="grid grid-cols-1 gap-4 lg:grid-cols-12 lg:gap-5">
      {/* product upsell */}
      <div className="relative overflow-hidden rounded-xl border border-primary/25 bg-wrap p-4 lg:col-span-5">
        <div
          aria-hidden
        ...
        <div className="relative">
          ...
          <h3 className="mt-2 text-[15px] font-semibold tracking-tight text-d-text-primary">Trade with the full AI desk</h3>
          <p className="mt-0.5 text-[11.5px] text-d-text-muted">The engines, the bot, and the debate — working your book while you sleep.</p>
          ...
          <Link
            href="/pricing"
            className="mt-3.5 inline-flex w-full items-center justify-center gap-1.5 rounded-lg bg-primary px-3 py-2 text-[12px] font-semibold text-primary-foreground transition-colors hover:bg-primary-hover"
          >
