          ...
          return (
            <div key={t.date} className="flex min-w-0 flex-1 flex-col items-center gap-1">
              <div className="flex h-9 w-full items-end justify-center">
                <div className={`w-full max-w-[16px] rounded-mark ${up ? 'bg-up/70' : 'bg-down/70'}`} style={{ height: `${h}px` }} />
              </div>
      ...
      <div className={`mt-4 grid grid-cols-1 items-stretch gap-3 ${isPost ? 'lg:grid-cols-2' : 'lg:grid-cols-3'}`}>
        ...
        {!isPost && (globalItems.length > 0 || gift?.last != null) && (
          <motion.div variants={item} className="tile-tint p-4 lg:col-span-2">
            ...
            {/* WRAPS, does not scroll. As a single `overflow-x-auto` row this
                clipped the sixth index off the right edge with no affordance,
                AND left the bottom two-thirds of the panel empty (the panel is
                col-span-2 and stretches to its taller neighbour). Wrapping into
                a grid fills the panel and makes every index visible at once —
