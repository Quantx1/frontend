...
export default function QuickTrade({
  ...
  useEffect(() => {
    ...
    debounceRef.current = setTimeout(async () => {
      setIsSearching(true)
      try {
        const res = await api.screener.searchInstruments(searchQuery, 8)
        setSearchResults(res.instruments.map(it => ({ symbol: it.symbol, name: it.name || it.symbol, price: 0 })))
      } catch {
            ...
            <div className="bg-background-surface border border-d-border rounded-2xl shadow-2xl w-full max-w-2xl max-h-[90vh] overflow-hidden">
              ...
              <div className="p-6 overflow-y-auto max-h-[calc(90vh-140px)]">
                ...
                <form onSubmit={handleSubmit(handleFormSubmit)} className="space-y-6">
                  {/* Stock Search */}
                  <div>
                    <label className="block text-sm font-medium text-d-text-secondary mb-2">
                      Search Stock
                    </label>
                        ...
                        {searchResults.map((stock) => (
                          ...
                          >
                            <div className="flex items-center justify-between">
                              <div>
                                <div className="font-bold text-d-text-primary">{stock.symbol}</div>
                                <div className="text-xs text-d-text-muted">{stock.name}</div>
                              </div>
                              <div className="text-sm font-mono text-d-text-primary">
                  ...
                  <div className="grid grid-cols-2 gap-4">
                    ...
                    <div>
                      <label className="block text-sm font-medium text-d-text-secondary mb-2 flex items-center justify-between">
                        <span>Quantity</span>
                        <button
                          type="button"
                    ...
                    <div className="bg-background-elevated rounded-xl p-4 border border-d-border">
                      <div className="grid grid-cols-3 gap-4 text-center">
                        ...
                        <div>
                          ...
                          <p className="text-lg font-bold text-d-text-primary">
